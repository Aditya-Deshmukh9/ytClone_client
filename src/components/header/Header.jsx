...
function Header({ setIsFulSidebar, isFulSidebar }) {
  ...
  return (
    <header className="fixed top-0 bg-[#0f0f0f] left-0 w-full z-50 flex justify-between items-center px-2 md:px-4 py-2 border-b border-[#272727]">
      <div className="flex items-center md:gap-x-4">
        <button
          onClick={() => setIsFulSidebar(!isFulSidebar)}
          className="cursor-pointer p-1 md:p-2 rounded-full hover:bg-[#272727] transition-colors"
        >
          <FiMenu size={22} className="text-white" />
        </button>
        <Link to="/" className="flex items-center gap-x-1">
          <img src="/yt.svg" alt="youtube_logo" />
        </Link>
      </div>
